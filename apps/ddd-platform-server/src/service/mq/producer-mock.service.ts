...
// 模拟RocketMQ Producer接口
class MockMQProducer {
  async send(message: { topic: string; tag: string; body: string; keys: string }): Promise<any> {
    console.log('Mock MQ Message sent:', message);
    return { msgId: `mock_${Date.now()}`, status: 'SUCCESS' };
  }

  async start(): Promise<void> {
    console.log('Mock MQ Producer started');
  }

  async shutdown(): Promise<void> {
    console.log('Mock MQ Producer shutdown');
  }
}
...
export class RocketMQProducerService {
  ...
  private producer!: MockMQProducer;

  @Init()
  async init() {
    // 使用模拟Producer，生产环境中应该使用真实的RocketMQ客户端
    this.producer = new MockMQProducer();
    await this.producer.start();
    console.log('RocketMQ Producer initialized successfully');
  }
